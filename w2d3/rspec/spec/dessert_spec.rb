...
describe Dessert do
  let(:chef) { double("chef", name: "Jamie") }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("warm", "one", :chef) }.to \
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ["milk", "eggs", "flour", "cacao"]
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Jamie the Great Baker")
      expect(dessert.serve).to eq("Chef Jamie the Great Baker has made 5 cakes!")
    end
  end
